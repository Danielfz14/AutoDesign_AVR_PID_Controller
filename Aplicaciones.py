...
class P18(BasicProblem):
        
    def __init__(self, variable_num):
        super().__init__(variable_num)
        self.max_search_range = np.array([2.0,2.0,2.0])
        self.min_search_range = np.array([0.2,0.2,0.2])     
    ...
    def get_func_val(self, variables, *args):
        alfa=0.7
        kp,ki,kd= variables[0], variables[1], variables[2]
        Gp=co.tf([0.1*kd ,(0.1*kp+10*kd), (0.1*ki+10*kp) ,10*ki],[0.0004 ,0.0454, 0.555 ,(1.51+10*kd) ,(1+10*kp) ,10*ki]);
        t,y=co.step_response(Gp)
        Hi = stepinfo(Gp)
        Ts=Hi['SettlingTime']
        L=Hi['Overshoot']
        Tr=Hi['RiseTime']
        ...
        if np.isnan(L):
            L=100
        fcost=np.exp(-0.8)*np.abs(L/100+(1-np.exp(-0.8))*np.abs(1-y[-1]))+np.abs(Ts-Tr);
        return fcost 
